angular
    .module("app.createProjectCtrl", ["ngMessages", "gp.rutValidator", "selectize"])
    .controller("createProjectCtrl", ["$scope", "$http", "$timeout", function($scope, $http, $timeout) { 
        ...
        */ 
        $scope.crearProyecto = function() { 
            ...
            //revisa todos los proyectos existente de un usuario
            for(var i = 0; i < $scope.miusuario.length; i++) { 
  			   
               //si el nombre del proyecto es igual al ingresado 
               if($scope.miusuario[i].name.toLowerCase() == $scope.name_project.toLowerCase() && 
                    $scope.miusuario[i].owner_email == $scope.mi_email) { 

                    // la bandera es verdadera, entonces no se manda el post
                    $scope.bandera = true;
                    break;
  			   }
  		    } 
            ...
            if($scope.bandera == false) { 
                ...
                $http({ 
                    ...
                    if(data.project == "false") { 
                        ...
                        //si el servidor correctamente el json
                        ...
                        $http({ 
                                ...
                                //si el POST es válido 
                                if(data_mensaje.mensaje == "false") { 
                                    // Si hay error manda mensaje 
                                    Materialize.toast($mensaje3, 5000); 
                                } else { 
                                    // En caso de realizar el post correctamente, realiza un nuevo POST para crear el dialogo 
                                    $http({ 
                                        method: 'POST', 
                                        url: '/dialogo/create/', 
                                        headers: {
                                        ...
                                        if(data_dialogo.dialogo=='false') {//en caso de error manda mensaje 
                                            ...
                                            $http({ 
                                                    ...
                                                    //en caos de existir algun error se deplega mensaje 
                ...
                //entonces el nombre existe y se manda un mensaje de error
